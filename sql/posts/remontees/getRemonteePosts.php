...
//Reqwest
$reqwest = $conn->prepare("SELECT posts.content, posts.date, users.login FROM posts JOIN users ON posts.id_user = users.id WHERE posts.id_remontee = ?;");
$reqwest->bind_param("s", $id_remontee);
$reqwest->execute();
